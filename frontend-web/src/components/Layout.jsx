...
function getPageTitle(pathname) {
  ...
  if (pathname.startsWith("/superadmin/settings/security")) return "Security Settings";
...
export default function Layout({ children }) {
  ...
  const canSeeSettings = user?.role === "superadmin" && hasPermission("rbac.role.view");
              ...
              {profileOpen && (
                <div className="absolute right-0 top-[calc(100%+8px)] w-60 layout-dropdown rounded-xl shadow-2xl z-50 border layout-border overflow-hidden">
                  ...
                  <div className="p-1.5">
                    ...
                    {canSeeSettings && (
                      <button
                        onClick={() => { setProfileOpen(false); navigate("/superadmin/settings/roles"); }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg layout-nav-idle transition-all text-left"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
