      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-wrap gap-3">
          <input
            ...
            className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm w-64 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <select
            value={filterStatus}
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {items.map(t => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs text-blue-700 dark:text-blue-400">{t.transfer_number}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-900 dark:text-white truncate max-w-[140px]">{t.asset_name || "—"}</div>
                      <div className="text-xs text-gray-400 dark:text-gray-500">{t.asset_number}</div>
                    </td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300 truncate max-w-[120px]">{t.from_assignee_name || t.from_employee_name || "—"}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300 truncate max-w-[120px]">{t.to_assignee_name || t.to_employee_name || "—"}</td>
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 text-xs">
                        <span>{TYPE_ICONS[t.transfer_type] || "🔄"}</span>
                        <span className="text-gray-600 dark:text-gray-300">{t.transfer_type?.replace(" Transfer","")}</span>
                      </span>
                      {t.is_temporary && (
