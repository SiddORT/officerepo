...
export default function AssetMasterList() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await assetMgmtApi.listMasters({ category_id: catFilter || undefined, sub_category_id: scFilter || undefined, search, status: statusFilter, page, page_size: PAGE_SIZE });
  ...
  const filteredSubCats = catFilter ? subCategories.filter(s => s.category_id === catFilter) : subCategories;

  const toggleStatus = async (m) => {
    try {
      if (m.is_active) await assetMgmtApi.deactivateMaster(m.id);
      else await assetMgmtApi.activateMaster(m.id);
      showToast(`Asset ${m.is_active ? "deactivated" : "activated"}.`);
      load();
    ...
    <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <p style={{ margin: "3px 0 0", fontSize: 12, color: "var(--c-muted)" }}>Standard asset templates — {total} total</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setScFilter(""); setPage(1); }}
        ...
        {filteredSubCats.length > 0 && (
          <select value={scFilter} onChange={e => { setScFilter(e.target.value); setPage(1); }}
      ...
      <div style={{ background: "var(--c-surface)", border: "1px solid var(--c-border)", borderRadius: 10, overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: "center", color: "var(--c-muted)", fontSize: 13 }}>
            ...
            <div style={{ fontSize: 12, marginBottom: 14 }}>{search || statusFilter || catFilter ? "Try adjusting your filters." : "Add your first asset master."}</div>
            {!search && !statusFilter && !catFilter && <button onClick={() => navigate("/superadmin/assets/masters/new")} style={{ padding: "8px 16px", borderRadius: 7, background: "var(--c-accent)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>+ Add Asset Master</button>}
          </div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
              ...
              <tbody>
                {rows.map((m, i) => (
                  <tr key={m.id} style={{ borderBottom: "1px solid var(--c-border)", cursor: "pointer", transition: "background 0.15s" }}
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        ...
                        <BoolBadge value={m.maintenance_tracking_enabled} label="Maintenance" />
                        {!m.serial_number_required && !m.warranty_tracking_enabled && !m.maintenance_tracking_enabled && <span style={{ opacity: 0.4, fontSize: 11 }}>—</span>}
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px" }}><StatusBadge active={m.is_active} /></td>
                    <td style={{ padding: "12px 16px" }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => navigate(`/superadmin/assets/masters/${m.id}`)} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 5, border: "1px solid var(--c-border)", background: "transparent", color: "var(--c-text2)", cursor: "pointer" }}>View</button>
                        <button onClick={() => toggleStatus(m)} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 5, border: `1px solid ${m.is_active ? "rgba(239,68,68,0.3)" : "rgba(34,197,94,0.3)"}`, background: "transparent", color: m.is_active ? "#f87171" : "#4ade80", cursor: "pointer" }}>{m.is_active ? "Deactivate" : "Activate"}</button>
                      </div>
                    </td>
                  </tr>
                ))}
